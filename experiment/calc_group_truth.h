...
public:
    ...
    DataLoader *data_loader, 
    DataLoader *query_data_loader, int K_) {
        K = K_;
    ...
    template<typename dist_t>
    double calc_recall(std::priority_queue<std::pair<dist_t, hnswlib::labeltype >> knn, int id) {
        std::vector<uint32_t> knn_id;
        while (knn.size()) {
            knn_id.push_back(knn.top().second);
            knn.pop();
        }
        return calc_recall(knn_id, id);
    }
    double calc_recall(std::vector<uint32_t> &knn, int id) {
